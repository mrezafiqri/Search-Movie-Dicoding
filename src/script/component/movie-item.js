  ...
  render() {
    ...
    this.innerHTML = `
      <a href="#" id="movieElement" class="relative flex flex-col rounded-md overflow-hidden border border-cyan-800 shadow-lg shadow-cyan-800 hover:shadow-xl hover:shadow-cyan-900 duration-300 group">
        <img src="https://image.tmdb.org/t/p/w500${this._movie.poster_path}" alt="Poster ${this._movie.title}" class="object-cover object-center group-hover:brightness-50 group-hover:blur-sm duration-300">
        <h2 class="line-clamp-1 py-1 px-2 text-sm font-inter font-[700] group-hover:text-cyan-500 duration-300 md:text-xl md:pt-2 md:px-4 lg:text-base">${this._movie.title}</h2>
        <p class="text-slate-400 px-2 pb-2 font-inter text-xs font-[300] md:text-base md:px-4 md:pb-3 lg:text-xs">${this._movie.release_date}</p>
      ...
      <div id="modal-container" class="modal z-50 fixed h-full w-full flex justify-center items-center top-0 left-0 right-0 mx-auto bg-black/70 md:h-full md:w-full">
        <div id="modal-card" class="overflow-y-auto container p-4 rounded-md absolute w-2/3 max-h-[90%] top-10 mx-4 bg-dark grid grid-cols-1  border border-cyan-800 shadow-lg shadow-cyan-800 cursor-default md:grid-cols-3 md:top-1/3 md:w-[85%] lg:w-[60%] lg:-translate-y-1/4 lg:mt-8">
            <img src="https://image.tmdb.org/t/p/w500${this._movie.poster_path}" alt="" class="object-cover object-center rounded-md border border-cyan-800">
        ...
        <a id="close-modal" href="#" class="overflow-visible absolute z-40 opacity-80 top-6 right-7 md:text-2xl md:right-10 md:top-20 lg:right-28 lg:top-20 hover:opacity-50">
        ❌
        </a>
      </div>
    `;
