  ...
  render() {
    ...
    this.innerHTML = `
      <a href="#" id="movieElement" class="relative flex flex-col rounded-md overflow-hidden border border-cyan-800 shadow-lg shadow-cyan-800 hover:shadow-xl hover:shadow-cyan-900 duration-300 group md:rounded-lg">
        ...
        <h2 class="line-clamp-1 py-1 px-3 text-sm font-inter font-[700] group-hover:text-cyan-500 duration-300 md:text-xl md:pt-2 md:px-4 lg:text-base 2xl:text-lg">${this._movie.title}</h2>
        <p class="text-slate-400 px-3 pb-2 font-inter text-xs font-[300] md:text-base md:px-4 md:pb-3 lg:text-xs 2xl:text-base">${this._movie.release_date}</p>
        ...
        <p class="${getColor(this._movie.vote_average)} absolute top-2 right-2 p-2 w-8 h-8 bg-primary/80 border border-cyan-800 font-[600] font-inter text-xs rounded-[50%] flex justify-center items-center 2xl:text-base 2xl:w-10 2xl:h-10">${Math.round(this._movie.vote_average * 10) / 10}</p>
      </a>
      ...
      <div id="modal-container" class="modal z-50 fixed h-full w-full flex justify-center items-center top-0 left-0 right-0 mx-auto bg-black/70 md:h-full md:w-full">
        <div id="modal-card" class="overflow-y-auto container p-4 rounded-lg absolute w-2/3 max-h-[90%] top-20 mx-4 bg-dark  grid grid-cols-1  border border-cyan-800 shadow-lg shadow-cyan-800 cursor-default md:grid-cols-3 md:top-1/3 md:w-[85%] lg:w-[60%] lg:-translate-y-1/4 lg:mt-8">
            ...
            <div class="md:col-span-2 md:px-4 lg:px-2">
              <h2 class="pt-2 px-2 text-base text-slate-200 font-inter font-[700] leading-5 md:text-2xl md:pt-0 md:px-4 md:col-span-2 2xl:text-3xl">${this._movie.title}</h2>
              <p class="text-slate-400 cursor-text px-2 py-2 font-inter text-xs font-[300] md:text-lg md:leading-6 md:px-4 md:pb-3 2xl:text-xl">${this._movie.overview}</p>
              <p class="text-slate-400 px-2 font-inter text-xs font-[300] md:text-base md:px-4 md:pb-3 2xl:text-xl">Rating: ${Math.round(this._movie.vote_average * 10) / 10}</p>
              <p class="text-slate-400 text-right px-2 pb-2 font-inter text-xs font-[300] md:text-base md:px-4 md:pb-3 lg:text-left 2xl:text-xl">${this._movie.release_date}</p>
            </div>
        </div>
        <a id="close-modal" href="#" class="overflow-visible absolute z-40 opacity-80 top-14 right-8 md:text-2xl md:right-10 md:top-20 lg:right-28 lg:top-20 hover:opacity-50 2xl:right-36 2xl:top-24">
        ❌
        </a>
      </div>
    `;
