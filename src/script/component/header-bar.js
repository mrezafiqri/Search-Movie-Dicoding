  ...
  render() {
    ...
    <header class="fixed bg-dark top-0 left-0 right-0 z-10 border-b-2 border-cyan-800">
      <nav class="flex relative justify-between items-center m-auto p-4 w-full md:w-11/12 md:px-0 lg:w-10/12">
        ...
        <div id="search-container" class="nav-search absolute container m-auto flex justify-end pr-4 mt-4 gap-4 top-full right-0 md:pr-0 lg:w-1/2">
    ...
    this.addEventListener('click', (e) => {
      const btnSearch = this.querySelector('#toggleSearchElement');
      const searchContainer = this.querySelector('#search-container');

      if (!btnSearch.contains(e.target) && !searchContainer.contains(e.target)) {
